...
Both topics are associated with a single HTS non-fungible token (the **ownership token**). The token's identifier (the `vault` field in message formats) is the stable identifier used to locate and authenticate the vault. Transfer of the token transfers all vault access rights.
...
- The **ownership token** (`vault`) anchors both topics. Its value is embedded in each topic memo as the stable vault identifier.
...
```json
{
  "p": "hcs-XX",
  "op": "create",
  "vault": "<HTS token identifier>",
  "t_id": "<HCS-1 topic ID of encrypted context>",
...
| `vault` | REQUIRED | HTS token identifier; must match vault topic memo |
...
```json
{
  ...
  "op": "update",
  "vault": "<HTS token identifier>",
  "t_id": "<HCS-1 topic ID of updated encrypted context>",
...
| `vault` | REQUIRED | HTS token identifier |
| `t_id` | REQUIRED | HCS-1 topic ID of updated context |
...
The transfer operation signals a change of ownership on-chain. The new holder can immediately open a session using their wallet credentials once the HTS token transfer is confirmed.

```mermaid
sequenceDiagram
    participant PrevHolder as Previous Holder
    participant HTS as Hedera Token Service
    participant VaultTopic as Vault Topic (HCS-1)
    participant AuditTopic as Audit Topic (HCS-2)
    participant NewHolder as New Holder

    PrevHolder->>HTS: Transfer ownership token to NewHolder
    HTS-->>PrevHolder: Transfer confirmed (token ID: vault)
    PrevHolder->>VaultTopic: Submit transfer message {op: "transfer", vault, from, to}
    PrevHolder->>AuditTopic: Submit log entry {op: "log", vault_op: "transfer", content_hash, sequence}
    Note over VaultTopic,AuditTopic: Vault content unchanged — only ownership changes
    NewHolder->>HTS: Verify token ownership on-chain
    NewHolder->>VaultTopic: Open session using wallet credentials
    NewHolder->>VaultTopic: Decrypt and access context
```
...
| 4 | Client | — | Submits `vault` token ID and wallet signature to session endpoint |
...
| 8 | Client | — | Derives encryption key from `vault` token ID and wallet signature |
...
- `vault` MUST match the Hedera token ID format: three groups of integers separated by periods (e.g., `0.0.12345`).
- `content_hash` MUST be a 64-character lowercase hexadecimal string (SHA-256).
- `t_id` (where present) MUST reference a valid HCS-1 topic ID.
